...
import org.dpdns.timerverse.ninjabrain.algorithm.statistics.DiscretizedDensity;
...
public class DivineContext implements IDivineContext {
	...
	public Fossil getFossil() {
		return currentFossil;
	}

	public void setFossil(Fossil fossil) {
		this.currentFossil = fossil;
		onFossilChanged();
	}

	@Override
	public double relativeDensity() {
		return currentFossil == null ? 1.0 : (16.0 / 3.0);
	}

	@Override
	public boolean hasDivine() {
		return currentFossil != null;
	}
	...
	public BlindPosition getClosestCoords(double x, double z, double r) {
		...
		int angleIndex = -4 + currentFossil.x;
		if (angleIndex < 0) {
			angleIndex += 16;
		}
